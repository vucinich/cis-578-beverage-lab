...
int main(int argc, char **argv) {
  ...
  // 4.5: get the beverage service's port and address
  int beverage_service_port = config_json["beverage-service"]["port"];
  std::string beverage_service_addr = config_json["beverage-service"]["addr"];

  // 5.5: get the client of beverage-service
  ClientPool<ThriftClient<BeverageServiceClient>> beverage_client_pool(
      "beverage-service", beverage_service_addr, beverage_service_port, 0, 128, 1000);
