...
class OrderBeverageServiceHandler : public OrderBeverageServiceIf {
 public:
  OrderBeverageServiceHandler(
      ...
      ClientPool<ThriftClient<BeverageServiceClient>> *) ;
 ...
 private:
  ...
  ClientPool<ThriftClient<BeverageServiceClient>> *_beverage_client_pool;
};
...
void OrderBeverageServiceHandler::PlaceOrder(std::string& _return, const int64_t city){
    ...
    // beverage client
    // 1. get the beverage service client pool
